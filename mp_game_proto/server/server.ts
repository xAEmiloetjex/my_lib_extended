...
const SERVER_SINGLE_IP_LIMIT = 69420;

const EXIT_ON_BREAK = true;
const ALLOW_NON_REMOTE = true;
...
function exitOnBreak(reason: string) {
    if (EXIT_ON_BREAK !== true) return;
    console.log(reason);
    process.exit(1);
}
...
wss.on("connection", (ws, req) => {
    ...
    if (players.size >= SERVER_TOTAL_LIMIT) {
        exitOnBreak('Server limit reached!');
    ...
    if (req.socket.remoteAddress === undefined && ALLOW_NON_REMOTE !== true) {
        // NOTE: something weird happened the client does not have a remote address
        exitOnBreak('Client doesn\'t have a remote address!');
...

    {
        ...
        if (count >= SERVER_SINGLE_IP_LIMIT) {
            exitOnBreak('SingleIP limit reached!');
    ...
    let player = {
        ...
        x: 0,
        y: 0,
        pos: new Vec(x,y).setSpace(Spaces.VEC2).allowUnsafe(true),
        moving: 0,
        newMoving: 0,
        hue,
        moved: false,
    ...
    player['x'] = player.pos.get('x') || 0;
    player['y'] = player.pos.get('y') || 0;
