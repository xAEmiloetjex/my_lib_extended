import { Vec, Spaces, VecTypes } from "../lib/vector/index.js";
...
export const DIRECTION_VECTORS = (() => {
    ...
    vectors[Direction.Left] = new Vec(-1, 0).setSpace(Spaces.VEC2).readonly(true);
    vectors[Direction.Right] = new Vec(1, 0).setSpace(Spaces.VEC2).readonly(true);
    vectors[Direction.Up] = new Vec(0, -1).setSpace(Spaces.VEC2).readonly(true);
    vectors[Direction.Down] = new Vec(0, 1).setSpace(Spaces.VEC2).readonly(true);
    return vectors;
})();
...
export function updatePlayer(player, deltaTime) {
    ...
    for (let dir = 0; dir < Direction.Count; dir += 1) {
        if ((player.moving >> dir) & 1) {
            dx += DIRECTION_VECTORS[dir].map().x;
            dy += DIRECTION_VECTORS[dir].map().y;
        }
    }
    const l = dx * dx + dy * dy;
    if (l !== 0) {
